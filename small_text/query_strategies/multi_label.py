...
class CategoryVectorInconsistencyAndRanking(QueryStrategy):
    ...
    def __init__(self, batch_size=2048, prediction_threshold=0.5, epsilon=1e-8):
        """
        Parameters
        ----------
        batch_size : int
    ...
    def _compute_scores(self, indices_unlabeled, y, proba):
        y_pred_unlabeled = (proba[indices_unlabeled] > self.prediction_threshold).astype(int)
    ...
    def _compute_vector_inconsistency(self, y, y_pred_unlabeled, num_classes):
        ...
        for batch_idx in np.array_split(np.arange(y_pred_unlabeled.shape[0]), num_batches, axis=0):
    ...
    def _distance(self, y_pred_unlabeled_sub, y_arr, num_classes, a, b, c, d,
        ...
        joint_entropy += (b + c) / num_classes \
            * (self._entropy(b, b + c) + self._entropy(c, b + c))
        ...
        entropy_distance /= joint_entropy
    ...
    def _entropy(self, numerator, denominator):
        return -numerator / denominator * np.log2(numerator / denominator + self.epsilon)
